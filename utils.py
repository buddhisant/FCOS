...
def nms(scores, bboxes, labels, nms_thresholds):
    ...
    for classid in exist_classes: #逐类的剔除冗余预测框
        ...
        iou_per_class = compute_ious(bbox_per_class,bbox_per_class) #计算每个bbox对其他bbox的iou
        iou_per_class = torch.triu(iou_per_class, diagonal=1) #由于计算出来的是对称矩阵，下三角部分是重复的，且不需要主对角线上的部分

        overlaped_index_per_class = iou_per_class > nms_thresholds
        deleted_index_per_class = index_per_class.new_zeros(iou_per_class.size(0),dtype=torch.bool)

        for i in range(iou_per_class.size(0)):

            if(deleted_index_per_class[i]): #表明当前bbox已经被之前的某个bbox利用nms过程删除掉了
                continue
            deleted_index_per_class = deleted_index_per_class | overlaped_index_per_class[i] #保留未被nms抑制且属于相同类别的预测框

        result_scores.append(score_per_class[~deleted_index_per_class])
        result_bboxes.append(bbox_per_class[~deleted_index_per_class])
        result_labels.append(label_per_class[~deleted_index_per_class])
